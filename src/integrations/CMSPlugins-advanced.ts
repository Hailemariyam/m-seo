...
export class CMSPlugins {
  ...
  static async getAnalytics(
    ...
    // Adapter for future implementation
    // const adapter = this.getAdapter(config.platform);
...
?>`;
  ...
  private static convertToCSV(contents: CMSContent[]): string {
    ...
    const headers = Object.keys(firstContent);
  ...
  private static parseXML(data: string): CMSContent[] {
    ...
    const contentMatches = data.match(regex);

    if (!contentMatches) return contents;

    for (const match of contentMatches) {
      const content: any = {};

      const fieldMatches = match.matchAll(/<(\w+)>(.*?)<\/\1>/g);
